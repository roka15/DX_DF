...
#pragma region gate
			...
			ELayerType::FrontObject);
		{
			...
			mr->mesh = Resources::Find<Mesh>(L"RectMesh");
			mr->material = Resources::Find<Material>(L"DefaultVInverterMaterial");
			...
			ELayerType::BackObject);
		{
			...
			tf->parent = GateUp->GetComponent<Transform>();
		}
			...
			ELayerType::BackObject);
		{
			...
			tf->parent = MGateRight->GetComponent<Transform>();
		}
			...
			ELayerType::BackObject);
		{
			...
			tf->parent = MGateLeft->GetComponent<Transform>();
		}
...
#pragma region hud/ui
			...
			ELayerType::UI);
		{
			...
			tf->parent = HPBase->GetComponent<Transform>();
		}
			...
			ELayerType::UI);
		{
			...
			tf->parent = MPBase->GetComponent<Transform>();
		}
		float startx = -0.18f;
		float starty = -0.26f;
		for (int i = 0; i < 2; i++)
		{
			for (int j = 0; j < 7; j++)
			{
					...
					ELayerType::UI);
				{
					...
					std::shared_ptr<Transform> tf = SkillQuickSlot->GetComponent<Transform>();
					tf->parent = HudBaseTF;
				}
			}
		}
		...
		for (int i = 0; i < 1; i++)
		{
			for (int j = 0; j < 6; j++)
			{
					...
					ELayerType::UI);
				{
					...
					std::shared_ptr<Transform> tf = ItemSlot->GetComponent<Transform>();
					tf->parent = HudBaseTF;
				}
			}
		}
			...
			ELayerType::UI);
		{
			...
			tf->parent = Icon1->GetComponent<Transform>();
		}
...
#pragma region obj
			...
			ELayerType::BackObject);
		{
			...
			tf->parent = EnchantBook->GetComponent<Transform>();
		}
